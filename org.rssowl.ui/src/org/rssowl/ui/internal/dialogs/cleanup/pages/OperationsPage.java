...
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteFeedsByLastUpdateOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteFeedsByLastVisitOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteFeedsDuplicatesOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteFeedsWithErrorOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteNewsAbstractOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteNewsByAgeOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteNewsByCountOperation;
import org.rssowl.ui.internal.dialogs.cleanup.operations.DeleteNewsWithStateReadOperation;
 ...
 * @author bpasero
 */
public final class OperationsPage extends WizardPage {
  ...
  public ArrayList<ICleanUpOperation> getOperations() {

    ArrayList<ICleanUpOperation> ops = new ArrayList<ICleanUpOperation>();

    /* Feed Operations */
    ops.add(new DeleteFeedsByLastVisitOperation(fDeleteFeedByLastVisitCheck.getSelection(), fMaxLastVisitSpinner.getSelection()));
    ops.add(new DeleteFeedsByLastUpdateOperation(fDeleteFeedByLastUpdateCheck.getSelection(), fMaxLastUpdateSpinner.getSelection()));
    ops.add(new DeleteFeedsWithErrorOperation(fDeleteConErrorFeedCheck.getSelection()));
    ops.add(new DeleteFeedsDuplicatesOperation(fDeleteDuplicateBookmarksCheck.getSelection()));

    // TODO: update when new sync engines will be added
    //    ops.add(new DeleteFeedsWithBrokenSynchronizationOperation(
    //        ));

    /* News Operations */
    ops.add(new DeleteNewsByCountOperation(fDeleteNewsByCountCheck.getSelection(), fMaxCountSpinner.getSelection()));
    ops.add(new DeleteNewsByAgeOperation(fDeleteNewsByAgeCheck.getSelection(), fMaxAgeSpinner.getSelection()));
    ops.add(new DeleteNewsWithStateReadOperation(fDeleteReadNewsCheck.getSelection()));

    DeleteNewsAbstractOperation.setKeepUnreadNews(fNeverDeleteUnreadNewsCheck.getSelection());
    DeleteNewsAbstractOperation.setKeepLabeledNews(fNeverDeleteLabeledNewsCheck.getSelection());

    return ops;
  }

  public void savePreferences(IPreferenceScope preferences) {
    for (ICleanUpOperation op : getOperations()) {
      op.savePreferences(preferences);
    }

    // disable cleaning up index until next time index issue is reported
    preferences.putBoolean(DefaultPreferences.CLEAN_UP_INDEX, false);

    // save preferences for keeping news
    DeleteNewsAbstractOperation.saveKeepNewsPreferences(preferences);
  }
}
